...
**모든 기능은 CLI에서 수치로 단언(assert)할 수 있어야만 존재로 인정한다** — GUI 데모용 가짜 기능을
막는 바닥선이다. 이건 에이전트(Claude)가 화면이 없어서 헤드리스로만 검증한다는 뜻이지, **사람이
GUI로 보면 안 된다는 뜻이 아니다.** 오히려 사람이 GUI로 눈으로 확인하는 것은 **교차검증**이라 환영한다
(에이전트=수치 단언, 사람=눈 확인, 같은 시스템을 두 창으로). 그래서 모든 관람 경로(`make ros-sim`+
`ros-attach`, `watch-ros`, `watch-*`)는 헤드리스 단언과 **별개로 항상 갖춘다**. 사용자 지적(2026-07-24):
*"너는 너대로 검증하고 나도 화면 보며 확인해야 교차검증이 되지."*
...
| | 내가 읽는 문서 (`DECISIONS`·`STATUS`·`PLAN`) | **사람이 읽는 문서 (`docs/spec.html`)** |
|---|---|---|
| 시간 | **쌓는다.** append-only, 틀린 것도 기록으로 남긴다 | **지금 상태만.** 바뀌면 **기존 걸 지우고 다시 쓴다** |
| 형식 | 긴 산문 OK. 맥락·근거·번복까지 다 적는다 | **표·짧은 단위.** 산문 덩어리 200자 넘으면 구조로 쪼갠다 |
| 목적 | 내가 이어서 작업하기 위한 정본 | 사람이 **읽고 판단**하기 위한 것 |

**실제로 어긴 적 있다**(2026-07-23): spec.html 에 콜아웃만 계속 덧붙여서 로드맵 노드가 732자
한 덩어리가 됐고, 원인이 바뀐 뒤에도 옛 설명이 그대로 남아 있었다. 사용자 지적:
*"장황하게 글만 써두면 뭐하냐. 바뀐 내용이면 기존 걸 지우고 정리해서 써야지."*

**spec.html 갱신할 때 체크**:
1. 이 내용이 **바뀐 것**인가? → 옛 문단을 **삭제**하고 새로 쓴다. 밑에 덧붙이지 마라.
2. 200자 넘는 산문 덩어리인가? → 표·`kv`·짧은 콜아웃으로 쪼갠다.
3. 수치가 있나? → 문장에 묻지 말고 **표**로. 명령(`make ...`)과 결과를 나란히.

**다이어그램은 inline SVG 로 그린다. mermaid 쓰지 마라.** mermaid 는 Artifact 호스트가 렌더해주는
것이라 `docs/spec.html` 을 **로컬 파일로 열면 그냥 텍스트로 보인다**(사용자가 그렇게 본다).
SVG 는 CSS 변수(`var(--ink)` 등)를 쓰면 라이트/다크도 되고 어디서든 그려진다.
...
**⚠️ 그래서 시뮬 타깃은 절대 동시에 돌리지 마라.** `clean-sim`(대부분 타깃의 선행 조건)이
`pkill ign gazebo` 라, 두 번째 명령이 **돌고 있던 첫 번째의 시뮬을 죽인다.** 증상이 고약하다 —
죽는 게 아니라 제어 루프가 낡은 추정으로 계속 달려서 **그럴듯한 틀린 숫자**가 나온다.
(2026-07-27 에 실제로 밟음: 관통 실행 중 `make straddle` 을 돌려 U턴 측정이 오염됐다.)
백그라운드로 하나 돌리는 중이면 **끝날 때까지 다른 시뮬 명령을 내지 마라.**
...
perception/        # ML 코드 + conda 환경(perception/condaenv, conda **3.10**). 학습 결과는 models/best.pt
...
- **인식 ROS 노드**: `scripts/env.sh perception/condaenv/bin/python <node>` (ROS 유지 + venv torch 우선).
...
- **에이전트(Claude)는 GUI 실행 금지** — `rviz2`, `gzclient`, `-g` 없는 `ign gazebo`(GUI가 뜬다).
  ...
  ROS 이관(038)으로 `ros_gz_bridge`가 상주하니 **rviz2 로 카메라·검출(/weeds)도 볼 수 있다**(로봇 머릿속).
