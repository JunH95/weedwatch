...
class Portal:
    ...
    # ── 몸통 케이스 (데크 바로 아래, 꽉 찬 본체) ─────────────────────────
    # AVO 처럼 태양광 상판 아래에 실제 케이스가 있다. 그 안에 배터리·컴퓨터가 들어가고,
    # 다리·캐리지·도구가 여기에 붙는다. 다리 4개 사이가 뚫린 "테이블"이 아니다.
    body_height: float = 0.14  # 케이스 높이
    body_inset: float = 0.06   # 데크 가장자리에서 케이스까지 (케이스가 데크보다 작음)

    # ── 다리 (몸통 케이스 → 고랑의 바퀴) ─────────────────────────────────
    leg_width: float = 0.06  # 다리 단면 (사각기둥)
