...
(에이전트=수치 단언, 사람=눈 확인, 같은 시스템을 두 창으로). 그래서 모든 관람 경로(`make sim-live`+
`field-attach`, `watch-*`)는 헤드리스 단언과 **별개로 항상 갖춘다**. 사용자 지적(2026-07-24):
...
- **에이전트(Claude)는 GUI 실행 금지** — `rviz2`, `gzclient`, `-g` 없는 `ign gazebo`(GUI가 뜬다).
  ...
  `make sim-live`(상주 GUI 시뮬)+`make field-attach`, `watch-*`. 사람이 데스크톱에서 직접 보는 건
  교차검증이라 권장. (rviz2 로 보려면 ign→ROS `ros_gz_bridge` 가 필요하지만, 우리 스택은 ign-transport
  직결이라 **Gazebo GUI 클라이언트(`ign gazebo -g`)가 브리지 없이 바로 붙는 저마찰 경로**다.)
