...
# 관통(자율 제초 실행)은 ROS 로 이관됨(DECISIONS 038) — 직결 field_run.py 삭제.
#   헤드리스 단언(에이전트) : make ros-skeleton
#   한 방 관람(사람, GUI)    : make watch-ros
#   상주 시뮬+붙이기(사람)   : 터미널1 make ros-sim  /  터미널2 make ros-attach
# (데스크톱 전용) skeleton.launch 를 gui:=true 로 — 로봇 주행+타격을 GUI 로 한 번 관람.
watch-ros: clean-sim
...
# 이미 떠 있는 ros-sim 에 코디네이터를 붙여 관통 주행 주입(시뮬 안 죽임).
ros-attach:
	@$(ENV) bash -c "source install/setup.bash && WW_ROOT=$(CURDIR) ros2 run weedwatch_coordinator coordinator_node"
