...
| 부품 | 실제 스펙 | 역할 |
|---|---|---|
| **RGB-D 카메라** | Intel RealSense D405 (RGB+깊이 정렬, 1280×720@30, 작동 7–50cm, 87° FOV) | sim down_cam+down_depth **하나로 통합**. 근접역이 우리 작업거리와 일치. 높이를 비전으로 |
...
| **실 깊이 노이즈·야외 IR** | sim depth 노이즈=0(낙관) | D405 근접 벤치 + 도메인 랜덤화 |
...
2. **카메라 높이 상향(0.145→0.35m)** — D405가 두 값 다 커버. 0.35m면 한 프레임이 두둑폭 90cm 커버
   (~19px/cm). 전방 암으로 카리지·툴 안 가리게 재설계(robot_body). STATUS 미결의 그 항목.
3. **sim 카메라 intrinsic을 D405에 맞춤** — 640×480/60° → 1280×720/87°. sim-to-real px/cm 유효화.
4. **버스 12V vs 24V** — 12V(기성팩·단순) 잠정 1안.
5. **구동 방식** — 브러시DC+RoboClaw(싸고 단순) vs BLDC 인휠+CAN(효율·수명).
6. **실물 질량 ~40kg 반영** — sim 관성/구동/배터리를 실질량으로 재튜닝, 또는 경량화로 30kg 근접.
7. **AI Hub 조명/배경 갭 대응** — 도메인 랜덤화에 조명·배경 포함할지, LED 터널 확정할지.
8. **정원 크기 vs 로봇 크기**(PLAN §10 미해결) — 128cm 30–40kg가 5평 텃밭엔 큼. Stage 4~5에서 확정.
9. 데크 면적 표기 통일(1.05m²).
