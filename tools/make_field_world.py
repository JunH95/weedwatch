...
<sdf version="1.9">
  <world name="robot_field_multi">
    ...
    <!-- IMU 를 살린다. 이게 없으면 URDF 에 imu 센서가 있어도 /robot/imu 가 발행조차 안 된다(실측).
         두둑 끝 회전에 필요하다 — 4륜 고정(스키드 스티어)이라 제자리 회전에서 바퀴가 긁혀
         휠 오도메트리 yaw 가 26°/회전 부풀고, 방위를 되잡을 수 있는 건 IMU 뿐이다(diag_uturn). -->
