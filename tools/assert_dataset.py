...
게이트:
  ...
  2. 모든 마스크 픽셀이 정확히 3클래스 색(흙 검정 / 작물 초록 / 잡초 빨강) — 양자화 확인
  3. 데이터셋 전체에 작물 픽셀 AND 잡초 픽셀이 둘 다 존재 (한쪽만 있으면 학습 불가)
...
CLASSES = {"흙(배경)": (0, 0, 0), "작물": (0, 255, 0), "잡초": (255, 0, 0)}
...
def main():
    ...
    if off_palette_total > 0:
        errs.append(f"마스크에 3클래스 밖 색 {off_palette_total} 픽셀 — 양자화 실패")
    if totals[1] == 0:
        errs.append("작물(초록) 픽셀이 0 — 마스크가 작물을 라벨 못함")
    if totals[2] == 0:
        errs.append("잡초(빨강) 픽셀이 0 — 마스크가 잡초를 라벨 못함")

    if errs:
        print("\n❌ 데이터셋 실패:\n    - " + "\n    - ".join(errs), file=sys.stderr)
        sys.exit(1)
    print("\n=== ✅ 데이터셋 통과 — RGB+마스크 쌍이 작물/잡초/흙을 픽셀단위로 가른다 ===")
