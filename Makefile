...
# Stage 5 Step B: 경사 6° + 흙덩이 위를 달리며 타격. 보정 OFF/ON A/B.
worlds/robot_strike.sdf: tools/make_strike_world.py
	@$(ENV) python3 tools/make_strike_world.py > worlds/robot_strike.sdf
strike-terrain: build/ww_cmd worlds/robot_strike.sdf clean-sim
	@$(ENV) python3 tools/strike_marks.py --world strike
	@$(ENV) python3 tools/strike_marks.py --world strike --correct
watch-terrain: build/ww_cmd worlds/robot_strike.sdf clean-sim
	@$(ENV) python3 tools/strike_marks.py --world strike --correct --gui
...
dashboard:
	@$(ENV) python3 tools/build_dashboard.py
	@echo "artifacts/dashboard.html — 브라우저로 열어 봄"
...
# ROS 이관 Phase 0: ign→ros_gz_bridge→rclpy 폐루프 증명 (ROS판 make drive). 설치 0.
ros-drive: worlds/robot_field_multi.sdf clean-sim
	@$(ENV) python3 tools/ros_drive_smoke.py

# ROS 이관 Phase 1: ww_cmd(직결)를 대체하는 rclpy 제어 노드 자가검증(주행+관절, ROS 토픽).
ros-control: worlds/robot_field_multi.sdf clean-sim
	@$(ENV) python3 tools/ww_control.py

# ROS 이관 Phase 2: 인식 ROS 노드 자가검증 (카메라 토픽→best.pt→/weeds, 파일IO 대체).
ros-percept: worlds/robot_field_multi.sdf clean-sim
	@$(ENV) perception/condaenv/bin/python perception/ww_perception_node.py --selftest

# ROS 이관 Phase 3: 관통을 ROS 노드 조합으로 (field_run 직결판 대체). 같은 field_run.json 산출.
field-run-ros: worlds/robot_field_multi.sdf clean-sim
	@$(ENV) python3 tools/field_run_ros.py
